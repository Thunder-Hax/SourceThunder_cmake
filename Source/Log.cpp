...
char g_logFile2[MAX_PATH];
char g_debugLogFile2[MAX_PATH];

#define CHARS_FOR_BUFF2 4096
#define CHARS_FOR_PARAMS2 3500

static bool useFileOutput2 = true;
...
void Log::Init(HMODULE hModule) {
	...
	if (GetModuleFileNameA(hModule, g_logFile, MAX_PATH) != 0) {
		...
		sprintf_s(chLogBuff, szTimestamp, "INIT: Thunder");
		if (slash != -1) {
			g_logFile[slash + 1] = '\0';
			strcpy_s(g_debugLogFile, g_logFile);
			strcat_s(g_debugLogFile, "Thunder.log");
			...
			OutputDebugStringA("INIT: Thunder");
		}
		else {
			useFileOutput = false;
		}
...
void Log::chat(HMODULE hModule) {
	memset(g_logFile2, 0, sizeof(g_logFile2));

	if (GetModuleFileNameA(hModule, g_logFile2, MAX_PATH) != 0) {
		size_t slash = -1;

		for (size_t i = 0; i < strlen(g_logFile2); i++) {
			if (g_logFile2[i] == '/' || g_logFile2[i] == '\\') {
				slash = i;
			}
		}
		char chLogBuff[CHARS_FOR_BUFF2];
		char szTimestamp[30];
		struct tm current_tm;
		time_t current_time = time(NULL);
		...
		localtime_s(&current_tm, &current_time);
		sprintf_s(szTimestamp, "[%02d:%02d:%02d] %%s\n", current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
		sprintf_s(chLogBuff, szTimestamp, "INIT: Thunder Chat");
		if (slash != -1) {
			g_logFile2[slash + 1] = '\0';
			strcpy_s(g_debugLogFile2, g_logFile2);
			strcat_s(g_debugLogFile2, "ThunderChat.log");

			FILE* file;
			if ((fopen_s(&file, g_debugLogFile2, "w")) == 0)
			{
				fprintf_s(file, "%s", chLogBuff);
				fclose(file);
			}
			OutputDebugStringA("INIT: Thunder Chat");
		}
		else {
			useFileOutput2 = false;
		}
	}
	else {
		useFileOutput2 = false;
...
void Log::MsgChat(const char* fmt, ...) {
	va_list va_alist;
	char chLogBuff[CHARS_FOR_BUFF2];
	char chParameters[CHARS_FOR_PARAMS2];
	char szTimestamp[30];
	struct tm current_tm;
	time_t current_time = time(NULL);
	...
	if (useFileOutput2) {
		FILE* file;
		if ((fopen_s(&file, g_debugLogFile2, "a")) == 0)
		{
			fprintf_s(file, "%s", chLogBuff);
			fclose(file);
		}
...
void Log::Error2(const char* fmt, ...) {
	va_list va_alist;
	char chLogBuff[CHARS_FOR_BUFF2];
	char chParameters[CHARS_FOR_PARAMS2];
	char szTimestamp[30];
...
void Log::Fatal2(const char* fmt, ...) {
	va_list va_alist;
	char chLogBuff[CHARS_FOR_BUFF2];
	char chParameters[CHARS_FOR_PARAMS2];
	char szTimestamp[30];
