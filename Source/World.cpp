...
//	}
	void RenderingCamera_set(Camera2 newCamera, bool ease)
	{
		if (newCamera.Handle() == 0)
		{
			CAM::RENDER_SCRIPT_CAMS(false, ease, 3000, 1, 0);
		}
		else
		{
			newCamera.SetActive(true);
			CAM::RENDER_SCRIPT_CAMS(true, ease, 3000, 1, 0);
		}
	}
//
...
//
	...
	{
		float height = 0.0f;
		GAMEPLAY::GET_GROUND_Z_FOR_3D_COORD2(position.x1, position.y1, 1000.0f, &height);
...
//	}
	...
	{
		return CAM::CREATE_CAM("DEFAULT_SCRIPTED_CAMERA", 1);
	}
	Camera2 CreateCamera(const Vector31& position, const Vector31& rotation, float fov)
	{
		...
		Camera2 cam = CAM::CREATE_CAM("DEFAULT_SCRIPTED_CAMERA", 1);
	...
	GTAped CreatePed(GTAmodel::Model model, Vector31 position, float heading, bool placeOnGround)
	{
		...
		if (placeOnGround)
		{
			position.z1 = World::GetGroundHeight(position) + model.Dim1().z1;//model.Dim2().z;
		}

		model.Load(3000);

		GTAentity ped = PED::CREATE_PED(26, model.hash, position.x1, position.y1, position.z1, heading, 1, 1);
	...
	GTAped CreatePed(GTAmodel::Model model, Vector31 position, const Vector31& rotation, bool placeOnGround)
	{
		...
		auto& ped = CreatePed(model, position, rotation.z1, false);
...
//
	GTAvehicle CreateVehicle(GTAmodel::Model model, Vector31 position, float heading, bool placeOnGround)
	{
		...
		GTAentity vehicle = VEHICLE::CREATE_VEHICLE(model.hash, position.x1, position.y1, position.z1, heading, 1, 1);
	...
	GTAvehicle CreateVehicle(GTAmodel::Model model, Vector31 position, const Vector31& rotation, bool placeOnGround)
	{
		...
		auto vehicle = CreateVehicle(model, position, rotation.z1, false);
	...
	GTAprop CreateProp(GTAmodel::Model model, Vector31 position, bool dynamic, bool placeOnGround)
	{
		...
		GTAentity prop = OBJECT::CREATE_OBJECT(model.hash, position.x1, position.y1, position.z1, 1, 1, dynamic);
...
//
	...
	{
		Hash hash = 0;
		PED::ADD_RELATIONSHIP_GROUP((PCHAR)groupName.c_str(), &hash);
...
//	}
	void SetRelationshipBetweenGroups(int relationship, Hash group1, Hash group2)
	{
		...
		PED::SET_RELATIONSHIP_BETWEEN_GROUPS(relationship, group2, group1);
	}
	void SetRelationshipBetweenGroups(int relationship, const std::string& group1, const std::string& group2)
	{
		Hash group1h = gethashkey::GET_HASH_KEY(group1);
		Hash group2h = gethashkey::GET_HASH_KEY(group2);

		PED::SET_RELATIONSHIP_BETWEEN_GROUPS(relationship, group1h, group2h);
		PED::SET_RELATIONSHIP_BETWEEN_GROUPS(relationship, group2h, group1h);
	}
...
//	}*/
	...
	{
		GTAplayer myPlayer = PLAYER::PLAYER_PED_ID()/*PLAYER_ID()*/;
		GTAentity myPed = PLAYER::PLAYER_PED_ID();
		...
		Vector31& camCoord = GameplayCamera::Position_get();
		Vector31 hitCoord = (GameplayCamera::DirectionFromScreenCentre_get() * 1000.0f) + camCoord;
...
//
	...
	{
		GRAPHICS::DRAW_LINE(startPos.x1, startPos.y1, startPos.z1, endPos.x1, endPos.y1, endPos.z1, colour.R, colour.G, colour.B, colour.A);
	}
	void DrawPoly(const Vector31& pos1, const Vector31& pos2, const Vector31& pos3, const RGBA1& colour)
	{
		GRAPHICS::DRAW_POLY(pos1.x1, pos1.y1, pos1.z1, pos2.x1, pos2.y1, pos2.z1, pos3.x1, pos3.y1, pos3.z1, colour.R, colour.G, colour.B, colour.A);
	}
	void DrawLightWithRange(const Vector31& position, const RgbS& colour, float range, float intensity)
	{
		GRAPHICS::DRAW_LIGHT_WITH_RANGE(position.x1, position.y1, position.z1, colour.R, colour.G, colour.B, range, intensity);
	}
