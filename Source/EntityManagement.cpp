...
namespace Spooner
{
	namespace EntityManagement
	{
		...
		{
			bool isOnTheLine = NETWORK::NETWORK_IS_IN_SESSION() != 0;
		...
		bool GetEntityThisEntityIsAttachedTo(GTAentity& from, GTAentity& to)
		{
			...
			if (from.IsAttached())
			{
				to = ENTITY::GET_ENTITY_ATTACHED_TO(from.Handle());
				return true;
			}
			to = 0;
			return false;
		...
		void set_vehicle_wheels_invisible(GTAvehicle vehicle, bool enable)
		{
			if (enable)
			{
				...
				WAIT(100);
				VEHICLE::_SET_VEHICLE_ENGINE_TORQUE_MULTIPLIER(vehicle.Handle(), DBL_MAX * DBL_MAX);
				VEHICLE::_SET_VEHICLE_ENGINE_POWER_MULTIPLIER(vehicle.Handle(), DBL_MAX * DBL_MAX);
				vehicle.ApplyForceRelative(Vector31(0, 0, -DBL_MAX * DBL_MAX));
				WAIT(100);
				if (g_multList_rpm.count(vehicle.Handle()))
				{
					VEHICLE::_SET_VEHICLE_ENGINE_POWER_MULTIPLIER(vehicle.Handle(), g_multList_rpm[vehicle.Handle()]);
				}
				else
				{
					VEHICLE::_SET_VEHICLE_ENGINE_POWER_MULTIPLIER(vehicle.Handle(), 1.0f);
				}
				if (g_multList_torque.count(vehicle.Handle()))
				{
					VEHICLE::_SET_VEHICLE_ENGINE_TORQUE_MULTIPLIER(vehicle.Handle(), g_multList_torque[vehicle.Handle()]);
				}
				else
				{
					VEHICLE::_SET_VEHICLE_ENGINE_TORQUE_MULTIPLIER(vehicle.Handle(), 1.0f);
				}
			}
			else
			{
				...
				VEHICLE::_SET_VEHICLE_ENGINE_TORQUE_MULTIPLIER(vehicle.Handle(), 0.0f);
				VEHICLE::_SET_VEHICLE_ENGINE_POWER_MULTIPLIER(vehicle.Handle(), 0.0f);
				WAIT(100);
				if (g_multList_rpm.count(vehicle.Handle()))
				{
					VEHICLE::_SET_VEHICLE_ENGINE_POWER_MULTIPLIER(vehicle.Handle(), g_multList_rpm[vehicle.Handle()]);
		...
		void AttachEntity(SpoonerEntity& ent, GTAentity to, int boneIndex, const Vector31& offset, const Vector31& rotation)
		{
			if (ent.Handle.Handle() != to.Handle())
			{
				...
				//ent.Handle.IsCollisionEnabled_set(bHasCollision);
				ENTITY::SET_ENTITY_LIGHTS(ent.Handle.Handle(), 0);
				if (wheelsAreInvis)
		...
		void ShowBoxAroundEntity(const GTAentity& ent, bool showPoly, RgbS colour)
		{
			if (ent.Exists())
			{
				...
				auto& boxUpperLeftRear = ent.GetOffsetInWorldCoords(-ent_md.Dim2.x1, -ent_md.Dim2.y1, ent_md.Dim2.z1);
				auto& boxUpperRightRear = ent.GetOffsetInWorldCoords(ent_md.Dim1.x1, -ent_md.Dim2.y1, ent_md.Dim2.z1);
				auto& boxLowerLeftRear = ent.GetOffsetInWorldCoords(-ent_md.Dim2.x1, -ent_md.Dim2.y1, -ent_md.Dim1.z1);
				auto& boxLowerRightRear = ent.GetOffsetInWorldCoords(ent_md.Dim1.x1, -ent_md.Dim2.y1, -ent_md.Dim1.z1);

				auto& boxUpperLeftFront = ent.GetOffsetInWorldCoords(-ent_md.Dim2.x1, ent_md.Dim1.y1, ent_md.Dim2.z1);
				auto& boxUpperRightFront = ent.GetOffsetInWorldCoords(ent_md.Dim1.x1, ent_md.Dim1.y1, ent_md.Dim2.z1);
				auto& boxLowerLeftFront = ent.GetOffsetInWorldCoords(-ent_md.Dim2.x1, ent_md.Dim1.y1, -ent_md.Dim1.z1);
				auto& boxLowerRightFront = ent.GetOffsetInWorldCoords(ent_md.Dim1.x1, ent_md.Dim1.y1, -ent_md.Dim1.z1);

				RGBA1& lineColour = colour.ToRGBA1(250);
				RGBA1& polyColour = colour.ToRGBA1(100);
