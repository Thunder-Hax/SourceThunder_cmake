...
//std::vector<GTAmodel::Model> g_vehHashes_LSUMMER;
...
//				{
//					Hash vehHash = MODEL::GET_HASH_KEY(nodeVeh.attribute("name").as_string());
...
//	{
//		/*{ VehicleClass::Summer, &g_vehHashes_LSUMMER },*/{ VehicleClass::Super, &g_vehHashes_SUPER },{ VehicleClass::Sport, &g_vehHashes_SPORT },{ VehicleClass::SportsClassic, &g_vehHashes_SPORTSCLASSIC },
...
{
	std::ifstream fin((GetPathffW(Pathff::ThunderMenu, true) + (L"PropList.txt")).c_str());
...
std::string get_ped_model_label(const GTAmodel::Model& model, bool properName)
{
	...
	{
		if (MODEL::GET_HASH_KEY(g_pedModels[i].first) == model.hash)
		{
...
std::string get_prop_model_label(const GTAmodel::Model& model)
{
	...
	{
		if (MODEL::GET_HASH_KEY(objectModels[i]) == model.hash)
		{
