	...
	namespace FileManagement
	{
		...
		void AddEntityToXmlNode(SpoonerEntity& e, pugi::xml_node& nodeEntity)
		{
			...
			else if (e.Type == EntityType::PED)
			{
				...
				nodePedStuff.append_child("HasShortHeight").text() = PED::GET_PED_CONFIG_FLAG(ep.Handle(), 223, false) != 0;
				...
				for (UINT8 i = 0; i <= 9; i++)
				{
					nodePedProps.append_child(("_" + std::to_string(i)).c_str()).text() = (std::to_string(PED::GET_PED_PROP_INDEX(ep.Handle(), i)) + "," + std::to_string(PED::GET_PED_PROP_TEXTURE_INDEX(ep.Handle(), i))).c_str();
				}
				for (UINT8 i = 0; i <= 11; i++)
				{
					nodePedComps.append_child(("_" + std::to_string(i)).c_str()).text() = (std::to_string(PED::GET_PED_DRAWABLE_VARIATION(ep.Handle(), i)) + "," + std::to_string(PED::GET_PED_TEXTURE_VARIATION(ep.Handle(), i))).c_str();
				}
				...
				if (sub::PedHeadFeatures_catind::DoesPedModelSupportHeadFeatures(eModel))
				{
					...
					if (sub::PedHeadFeatures_catind::vPedHeads.count(ep.Handle()))
					{
						...
						for (int i = 0; i < pedHead.overlayData.size(); i++)
						{
							...
							nodePedHeadOverlay.append_attribute("index") = PED::_GET_PED_HEAD_OVERLAY_VALUE(ep.Handle(), i);
				...
				bool isUsingScenario = PED::IS_PED_USING_SCENARIO(ep.Handle(), const_cast<PCHAR>(e.LastAnimation.name.c_str())) != 0;
				bool isPlayingAnim = ENTITY::IS_ENTITY_PLAYING_ANIM(ep.Handle(), const_cast<PCHAR>(e.LastAnimation.dict.c_str()), const_cast<PCHAR>(e.LastAnimation.name.c_str()), 3) != 0;
		...
		//		VEHICLE::GET_VEHICLE_MOD_COLOR_1(ev.Handle(), &mod1a, &mod1b, &mod1c);
		//		int mod2a, mod2b;
		//		VEHICLE::GET_VEHICLE_MOD_COLOR_2(ev.Handle(), &mod2a, &mod2b);
		...
		SpoonerEntityWithInitHandle SpawnEntityFromXmlNode(pugi::xml_node& nodeEntity, std::unordered_set<Hash>& vModelHashes)
		{
			...
			BOOL bNetworkIsGameInProgress = NETWORK::NETWORK_IS_GAME_IN_PROGRESS();
			...
			Vector31 placingEpos;
			Vector31 placingErot;
			placingEpos.x1 = nodeEntityPosRot.child("X").text().as_float();
			placingEpos.y1 = nodeEntityPosRot.child("Y").text().as_float();
			placingEpos.z1 = nodeEntityPosRot.child("Z").text().as_float();
			placingErot.x1 = nodeEntityPosRot.child("Pitch").text().as_float();
			placingErot.y1 = nodeEntityPosRot.child("Roll").text().as_float();
			placingErot.z1 = nodeEntityPosRot.child("Yaw").text().as_float();
			...
			if (e.e.Type == EntityType::PROP)
			{
				...
				if (e.e.TextureVariation != -1)
					OBJECT::_SET_OBJECT_TEXTURE_VARIANT(eo.Handle(), e.e.TextureVariation);
			}
			else if (e.e.Type == EntityType::PED)
			{
				...
				PED::SET_PED_RAGDOLL_ON_COLLISION(ep.Handle(), nodePedStuff.child("CanRagdoll").text().as_bool(false));

				if (nodePedStuff.child("HasShortHeight").text().as_bool()) PED::SET_PED_CONFIG_FLAG(ep.Handle(), 223, 1);
				...
				AI::SET_PED_PATH_CAN_USE_CLIMBOVERS(ep.Handle(), true);
				AI::SET_PED_PATH_CAN_USE_LADDERS(ep.Handle(), true);
				AI::SET_PED_PATH_CAN_DROP_FROM_HEIGHT(ep.Handle(), true);
				//SET_PED_PATH_PREFER_TO_AVOID_WATER(ep.Handle(), true);
				//SET_PED_PATH_AVOID_FIRE(ep.Handle(), true);
				PED::SET_PED_COMBAT_ABILITY(ep.Handle(), 2);
				PED::SET_PED_COMBAT_MOVEMENT(ep.Handle(), 2);

				PED::SET_PED_CAN_PLAY_AMBIENT_ANIMS(ep.Handle(), true);
				PED::SET_PED_CAN_PLAY_AMBIENT_BASE_ANIMS(ep.Handle(), true);
				PED::SET_PED_CAN_PLAY_GESTURE_ANIMS(ep.Handle(), true);
				PED::SET_PED_CAN_PLAY_VISEME_ANIMS(ep.Handle(), true, TRUE);
				PED::SET_PED_IS_IGNORED_BY_AUTO_OPEN_DOORS(ep.Handle(), true);
				...
				for (auto& nodePedCompsObject = nodePedComps.first_child(); nodePedCompsObject; nodePedCompsObject = nodePedCompsObject.next_sibling())
				{
					...
					PED::SET_PED_COMPONENT_VARIATION(ep.Handle(), pedCompId, stoi(pedCompIdValueStr.substr(0, pedCompIdValueStr.find(","))), stoi(pedCompIdValueStr.substr(pedCompIdValueStr.find(",") + 1)), 0);
				}
				for (auto& nodePedPropsObject = nodePedProps.first_child(); nodePedPropsObject; nodePedPropsObject = nodePedPropsObject.next_sibling())
				{
					...
					PED::SET_PED_PROP_INDEX(ep.Handle(), pedPropId, stoi(pedPropIdValueStr.substr(0, pedPropIdValueStr.find(","))), stoi(pedPropIdValueStr.substr(pedPropIdValueStr.find(",") + 1)), bNetworkIsGameInProgress);
				}
				...
				{
					PED::SET_PED_CONFIG_FLAG(ep.Handle(), stoi(std::string(nodePedConfigFlagsObject.name()).substr(1)), nodePedConfigFlagsObject.text().as_bool());
				}
				...
				if (sub::PedHeadFeatures_catind::DoesPedModelSupportHeadFeatures(eModel) && nodePedHeadFeatures)
				{
					...
					if (nodePedHeadFeatures.attribute("WasInArray").as_bool())
					{
						...
						PED::_SET_PED_HAIR_COLOR(ep.Handle(), pedHead.hairColour, pedHead.hairColourStreaks);
						PED::_SET_PED_EYE_COLOR(ep.Handle(), SYSTEM::ROUND((float)pedHead.eyeColour)); // Sjaak says so
						...
						for (auto& nodePedFacialFeature = nodePedFacialFeatures.first_child(); nodePedFacialFeature; nodePedFacialFeature = nodePedFacialFeature.next_sibling())
						{
							...
							PED::_SET_PED_FACE_FEATURE(ep.Handle(), ii, pedHead.facialFeatureData[ii]);
						}
						...
						for (auto& nodePedHeadOverlay = nodePedHeadOverlays.first_child(); nodePedHeadOverlay; nodePedHeadOverlay = nodePedHeadOverlay.next_sibling())
						{
							...
							PED::SET_PED_HEAD_OVERLAY(ep.Handle(), ii, overlayData_index, pedHead.overlayData[ii].opacity);
							PED::_SET_PED_HEAD_OVERLAY_COLOR(ep.Handle(), ii, sub::PedHeadFeatures_catind::GetPedHeadOverlayColourType((PedHeadOverlay)ii), pedHead.overlayData[ii].colour, pedHead.overlayData[ii].colourSecondary);
						}
				...
				if (nodePedTattooLogoDecals)
				{
					...
					for (auto& nodeDecal = nodePedTattooLogoDecals.first_child(); nodeDecal; nodeDecal = nodeDecal.next_sibling())
					{
						...
						PED::_SET_PED_DECORATION(ep.Handle(), decal.collection, decal.value);
					}
				}
				...
				if (nodeMovGrpName)
				{
					...
					PED::SET_PED_MOVEMENT_CLIPSET2(ep.Handle(), const_cast<PCHAR>(movGrpName.c_str()), 0x3E800000);
				...
				if (nodeWmovGrpName)
				{
					...
					PED::SET_PED_WEAPON_MOVEMENT_CLIPSET(ep.Handle(), const_cast<PCHAR>(wmovGrpName.c_str()));
					g_pedList_wmovGrp[ep.Handle()] = wmovGrpName;
				}
				if (nodeMovGrpName || nodeWmovGrpName)
				{
					WAIT(30);
					Vector31& coord = ep.Position_get();
					ENTITY::SET_ENTITY_COORDS(ep.Handle(), coord.x1, coord.y1, coord.z1 + 0.05f, 0, 0, 0, 1);
					ENTITY::FREEZE_ENTITY_POSITION(ep.Handle(), !bFrozenPos);
				}
				...
				if (nodeFacialMood)
				{
					sub::ComponentChanger_Outfit_catind::set_ped_facial_mood(ep, nodeFacialMood.text().as_string());
				}
