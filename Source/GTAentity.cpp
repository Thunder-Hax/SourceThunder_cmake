...
{
	return ENTITY::GET_ENTITY_TYPE(this->mHandle);
}

GTAblip GTAentity::CurrentBlip() const
{
	return UI::GET_BLIP_FROM_ENTITY(this->mHandle);
}
...
void GTAentity::FreezePosition(bool value)
{
	...
	ENTITY::FREEZE_ENTITY_POSITION(this->mHandle, value);
}

void GTAentity::Dynamic_set(bool value)
{
	ENTITY::SET_ENTITY_DYNAMIC(this->mHandle, value);
	if (value)
	{
		ENTITY::SET_ENTITY_VELOCITY(this->mHandle, 0.0f, 0.0f, 0.0f);
	}
}
...
{
	ENTITY::SET_ENTITY_HEALTH(this->mHandle, value);
}
...
{
	Vector31& pos = this->Position_get();
	GAMEPLAY::GET_GROUND_Z_FOR_3D_COORD2(pos.x1, pos.y1, 1100.0f, &pos.z1);
	return pos.z1;
}
void GTAentity::PlaceOnGround()
{
	...
	RaycastResult& ray1 = RaycastResult::Raycast(pos, Vector31(0, 0, -1.0f), 10000.0f, IntersectOptions::Map);
	if (ray1.DidHitAnything())
	{
		this->Position_set(ray1.HitCoords() + Vector31(0, 0, this->Dim1().z1));
		return;
	}
	RaycastResult& ray2 = RaycastResult::Raycast(pos, Vector31(0, 0, 1.0f), 10000.0f, IntersectOptions::Map);
	if (ray2.DidHitAnything())
	{
		this->Position_set(ray2.HitCoords() + Vector31(0, 0, this->Dim1().z1));
		return;
	}

	pos.z1 = this->GetGroundZ() + this->Dim1().z1;
	//NETWORK_REQUEST_CONTROL_OF_ENTITY(this->mHandle);
	ENTITY::SET_ENTITY_COORDS(this->mHandle, pos.x1, pos.y1, pos.z1, 0, 0, 0, 1);
	OBJECT::PLACE_OBJECT_ON_GROUND_PROPERLY(this->mHandle);
}
//
...
{
	return ENTITY::IS_ENTITY_DEAD(this->mHandle) != 0;
}
...
{
	return ENTITY::IS_ENTITY_A_PED(this->mHandle) != 0;
}
...
{
	ENTITY::SET_ENTITY_AS_MISSION_ENTITY(this->mHandle, value, 1);
}
//
...
{
	return ENTITY::IS_ENTITY_VISIBLE(this->mHandle) != 0;
}
void GTAentity::SetVisible(bool value)
{
	ENTITY::SET_ENTITY_VISIBLE2(this->mHandle, value);
}
//
...
{
	ENTITY::SET_ENTITY_MAX_HEALTH(this->mHandle, value);
}

GTAmodel::Model GTAentity::Model() const
{
	return ENTITY::GET_ENTITY_MODEL(this->mHandle);
}
GTAmodel::ModelDimensions GTAentity::ModelDimensions() const
{
	return this->Model().Dimensions();
}
void GTAentity::ModelDimensions(Vector31& dim1, Vector31& dim2) const
{
	this->Model().Dimensions(dim1, dim2);
}
Vector31 GTAentity::Dim1() const
{
	return this->Model().Dimensions().Dim1;
}
Vector31 GTAentity::Dim2() const
{
	return this->Model().Dimensions().Dim2;
}


Vector31 GTAentity::Position_get() const
{
	return ENTITY::GET_ENTITY_COORDS_VECTORT(this->mHandle, 1);
}
void GTAentity::Position_set(Vector31 value)
{
	ENTITY::SET_ENTITY_COORDS_NO_OFFSET(this->mHandle, value.x1, value.y1, value.z1, 1, 1, 1);
}
//
Vector31 GTAentity::Rotation_get() const
{
	return ENTITY::GET_ENTITY_ROTATION2(this->mHandle, 2);
}
void GTAentity::Rotation_set(Vector31 value)
{
	ENTITY::SET_ENTITY_ROTATION(this->mHandle, value.x1, value.y1, value.z1, 2, 1);
}
//
...
{
	return ENTITY::GET_ENTITY_ALPHA(this->mHandle);
}
void GTAentity::Alpha_set(int value)
{
	ENTITY::SET_ENTITY_ALPHA(this->mHandle, value, 0);
}
void GTAentity::ResetAlpha()
{
	ENTITY::RESET_ENTITY_ALPHA(this->mHandle);
}
...
void GTAentity::HasGravity_set(bool value)
{
	...
	ENTITY::SET_ENTITY_HAS_GRAVITY(this->mHandle, value);
}
//
...
{
	return !ENTITY::_IS_ENTITY_COLLISON_DISABLED(this->mHandle);
}
void GTAentity::IsCollisionEnabled_set(bool value)
{
	ENTITY::SET_ENTITY_COLLISION(this->mHandle, value, false);
}

//
int GTAentity::NetID() const
{
	return NETWORK::NETWORK_GET_NETWORK_ID_FROM_ENTITY(this->mHandle);
}
//
...
//}
Vector31 GTAentity::GetOffsetInWorldCoords(Vector31 offset) const
{
	return ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS1(this->mHandle, offset.x1, offset.y1, offset.z1);
}
Vector31 GTAentity::GetOffsetInWorldCoords(float X, float Y, float Z) const
{
	return ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS1(this->mHandle, X, Y, Z);
}
...
{
	return ENTITY::IS_ENTITY_ATTACHED(this->mHandle) != 0;
}
...
{
	ENTITY::DETACH_ENTITY(this->mHandle, 1, 1);
}
void GTAentity::AttachTo(GTAentity entity, int boneIndex, bool collision)
{
	this->AttachTo(entity.Handle(), boneIndex, collision, Vector31(0, 0, 0), Vector31(0, 0, 0));
}
void GTAentity::AttachTo(GTAentity entity, int boneIndex, bool collision, Vector31 position, Vector31 rotation)
{
	ENTITY::ATTACH_ENTITY_TO_ENTITY(this->mHandle, entity.Handle(), boneIndex, position.x1, position.y1, position.z1, rotation.x1, rotation.y1, rotation.z1, FALSE, FALSE, collision, FALSE, 2, TRUE);
}
void GTAentity::AttachTo(GTAentity entity, int boneIndex, Vector31 position, Vector31 rotation, bool b9, bool useSoftPinning, bool collision, bool isPed, int vertexIndex, bool fixedRot)
{
	ENTITY::ATTACH_ENTITY_TO_ENTITY(this->mHandle, entity.Handle(), boneIndex, position.x1, position.y1, position.z1, rotation.x1, rotation.y1, rotation.z1, b9, useSoftPinning, collision, isPed, vertexIndex, fixedRot);
}
...
//}
void GTAentity::ApplyForceRelative(Vector31 direction, ForceType forceType)
{
	this->ApplyForceRelative(direction, Vector31(0, 0, 0), forceType);
}
void GTAentity::ApplyForceRelative(Vector31 direction, Vector31 offset, ForceType forceType)
{
	ENTITY::APPLY_FORCE_TO_ENTITY(this->mHandle, static_cast<int>(forceType), direction.x1, direction.y1, direction.z1, offset.x1, offset.y1, offset.z1, false, true, true, true, false, true);
}
...
{
	return NETWORK::NETWORK_HAS_CONTROL_OF_ENTITY(this->mHandle) != 0;
}
bool GTAentity::RequestControlOnce()
{
	if (!NETWORK::NETWORK_IS_IN_SESSION() || this->HasControl())
		return true;
...
void GTAentity::Delete(bool tele)
{
	...
	if (tele) ENTITY::SET_ENTITY_COORDS_NO_OFFSET(this->mHandle, 32.2653f, 7683.5249f, 0.5696f, 0, 0, 0);
	...
	{
	case EntityType::PROP: OBJECT::DELETE_OBJECT(&handle); break;
	case EntityType::VEHICLE: VEHICLE::DELETE_VEHICLE(&handle); break;
	case EntityType::PED: PED::DELETE_PED(&handle); break;
	default: ENTITY::DELETE_ENTITY(&handle); break;
	}
}
bool GTAentity::Exists() const
{
	return ENTITY::DOES_ENTITY_EXIST(this->mHandle) != 0;
}
void GTAentity::NoLongerNeeded()
{
	ENTITY::SET_ENTITY_AS_NO_LONGER_NEEDED(&this->mHandle);
	//delete this;
}
