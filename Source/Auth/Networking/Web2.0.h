...
namespace net
{
	...
	public:
		...
		std::wstring Post2(bool header, wstring url, const char * postdata, ...)
		{
			static char buffer[32768];
		...
		std::wstring Get2(bool header, wstring url)
		{
			// Set URL.
			WinHttpClient client(url);
		...
		bool DownLoadFile2(const wstring url, const wstring physicalalloc)
		{
			WinHttpClient downloadClient(url);
