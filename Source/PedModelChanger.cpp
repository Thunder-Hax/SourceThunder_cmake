...
namespace sub
{
	namespace PedFavourites_catind
	{
		...
		bool IsPedAFavourite(GTAmodel::Model model)
		{
			...
			if (doc.load_file((const char*)(GetPathffA(Pathff::ThunderMenu, true) + xmlFavouritePeds).c_str()).status != pugi::status_ok)
				return false;
		...
		bool AddPedToFavourites(GTAmodel::Model model, const std::string& customName)
		{
			...
			if (doc.load_file((const char*)(GetPathffA(Pathff::ThunderMenu, true) + xmlFavouritePeds).c_str()).status != pugi::status_ok)
			{
				...
				doc.save_file((const char*)(GetPathffA(Pathff::ThunderMenu, true) + xmlFavouritePeds).c_str());
			}
			pugi::xml_node nodeRoot = doc.document_element();
			...
			return (doc.save_file((const char*)(GetPathffA(Pathff::ThunderMenu, true) + xmlFavouritePeds).c_str()));
		}
	...
	void ChangeModel_(GTAmodel::Model model)
	{
		if (model.IsInCdImage())
		{
			if (model.Load(4000))
			{
				GTAped playerPed = /*PLAYER_PED_ID()*/Features::playerme;
				...
				GTAentity att;
				auto spi = Spooner::EntityManagement::GetEntityIndexInDb(playerPed);
				if (spi >= 0)
				{
					auto& spe = Spooner::Databases::EntityDb[spi];
				...
				PLAYER::SET_PLAYER_MODEL(/*PLAYER_ID()*/Features::playerme, model.hash);

				playerPed = /*PLAYER_PED_ID()*/Features::Online::selectedPlayer;

				playerPed.IsCollisionEnabled_set(bHasCollision);

				//SET_PED_DEFAULT_COMPONENT_VARIATION(playerPed.Handle());
				PED::SET_PED_RANDOM_COMPONENT_VARIATION(playerPed.Handle(), 0);
