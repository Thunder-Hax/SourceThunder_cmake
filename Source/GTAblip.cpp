...
{
	return (uint8_t)UI::GET_BLIP_ALPHA(this->mHandle);
}
void GTAblip::SetAlpha(uint8_t value)
{
	UI::SET_BLIP_ALPHA(this->mHandle, value);
}

int GTAblip::Colour() const
{
	return UI::GET_BLIP_COLOUR(this->mHandle);
}
void GTAblip::SetColour(int value)
{
	UI::SET_BLIP_COLOUR(this->mHandle, value);
}

bool GTAblip::IsFlashing() const
{
	return UI::IS_BLIP_FLASHING(this->mHandle) != 0;
}
void GTAblip::SetFlashing(bool value)
{
	UI::SET_BLIP_FLASHES(this->mHandle, value);
}

void GTAblip::SetFriendly(bool value)
{
	UI::SET_BLIP_AS_FRIENDLY(this->mHandle, value);
}

bool GTAblip::IsOnMinimap() const
{
	return UI::IS_BLIP_ON_MINIMAP(this->mHandle) != 0;
}

bool GTAblip::IsShortRange() const
{
	return UI::IS_BLIP_SHORT_RANGE(this->mHandle) != 0;
}
void GTAblip::SetShortRange(bool value)
{
	UI::SET_BLIP_AS_SHORT_RANGE(this->mHandle, value);
}

Vector31 GTAblip::Position_get() const
{
	//return infoid ? GET_BLIP_INFO_ID_COORD(this->mHandle) : GET_BLIP_COORDS(this->mHandle);
	return UI::GET_BLIP_COORDS2(this->mHandle);
}
void GTAblip::Position_set(const Vector31& value)
{
	UI::SET_BLIP_COORDS(this->mHandle, value.x1, value.y1, value.z1);
}

void GTAblip::Rotation_set(float valueInDegrees)
{
	UI::SET_BLIP_ROTATION(this->mHandle, valueInDegrees);
}

void GTAblip::SetScale(float value)
{
	UI::SET_BLIP_SCALE(this->mHandle, value);
}

void GTAblip::ShowRoute(bool value)
{
	UI::SET_BLIP_ROUTE(this->mHandle, value);
}

int GTAblip::Icon() const
{
	return UI::GET_BLIP_SPRITE(this->mHandle);
}
void GTAblip::SetIcon(int value)
{
	UI::SET_BLIP_SPRITE(this->mHandle, value);
}
...
{
	return UI::GET_BLIP_INFO_ID_TYPE(this->mHandle);
}

void GTAblip::HideNumber()
{
	UI::HIDE_NUMBER_ON_BLIP(this->mHandle);
}
void GTAblip::ShowNumber(int number)
{
	UI::SHOW_NUMBER_ON_BLIP(this->mHandle, number);
}

bool GTAblip::Exists() const
{
	return UI::DOES_BLIP_EXIST(this->mHandle) != 0;
}
void GTAblip::Remove()
{
	if (UI::DOES_BLIP_EXIST(this->mHandle))
	{
