...
static inline void nativePush(T val)
{
	if (sizeof(T) > sizeof(UINT64))
	{
		throw std::runtime_error("Error: value size > 64 bit");
	}
...
inline T Invoke(UINT64 hash, Args&& ...args)
{
	// Initialisation de l'appel natif
	nativeInit(hash);
	using namespace CallerNative;
	g_NativeInvoker->BeginCallImpl();
	// Pousser les arguments
	//nativePush(std::forward<Args>(args)), ...);
	/*(g_NativeInvoker->PushArg(std::forward<Args>(args)), ...);*/
	/*(nativePush(std::forward<Args>(args)), ...);*/
	(g_NativeInvoker->PushArg(std::forward<Args>(args)), ...);
	g_NativeInvoker->EndCallImpl(hash);
	if constexpr (!std::is_void_v<T>)
	{
		return g_NativeInvoker->GetReturnValue<T>();
		/*return g_NativeInvoker->GetReturnValue<ReturnType>();*/
		////return *reinterpret_cast<ReturnType*>(g_NativeInvoker->GetReturnValue());
	}
}

//(ctx->push_arg(std::forward<Args>(args)), ...);
//return *ctx->get_return_value<T>();
//(ctx->Push(std::forward<Args>(args)), ...);
//return *ctx->GetResult<T>();


//template<typename ReturnType, typename ...Args>
//inline ReturnType Invoke(scrNativeHash hash, Args&& ...args)
//{
//	using namespace CallerNative;
//	g_NativeInvoker->BeginCallImpl();
//	(g_NativeInvoker->PushArg(std::forward<Args>(args)), ...);
