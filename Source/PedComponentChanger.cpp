...
//
	...
	namespace ComponentChanger_Outfit_catind
	{
		...
		bool Create(GTAentity ped, std::string filePath)
		{
			...
			Spooner::FileManagement::AddEntityToXmlNode(eped, nodeEntity);
			...
			for (auto& e : Spooner::Databases::EntityDb)
			{
				if (e.AttachmentArgs.isAttached)
				{
					GTAentity att;
					if (Spooner::EntityManagement::GetEntityThisEntityIsAttachedTo(e.Handle, att))
					{
						if (att.Handle() == ped.Handle())
						{
							auto& nodeAttachment = nodeAttachments.append_child("Attachment");
							Spooner::FileManagement::AddEntityToXmlNode(e, nodeAttachment);
						}
					}
				}
			}
		...
		void set_ped_facial_mood(GTAentity ped, const std::string& animName)
		{
			...
			m = animName;
			PED::SET_FACIAL_IDLE_ANIM_OVERRIDE(ped.Handle(), const_cast<PCHAR>(animName.c_str()), 0);
		}
		...
		bool Apply(GTAped ep, const std::string& filePath, bool applyModelAndHead, bool applyProps, bool applyComps, bool applyDecals, bool applyDamageTextures, bool applyAttachedEntities)
		{
			...
			{
				if (ep.Handle() == /*PLAYER_PED_ID()*/Features::Online::selectedPlayer)
				{
					bool bWas241 = (/*Static_241*/Features::playerme == ep.Handle());
					ChangeModel_(eModel);
					ep = /*PLAYER_PED_ID()*/Features::Online::selectedPlayer;
					if (bWas241) /*Static_241*/Features::playerme = ep.Handle();
				}
			...
			if (nodeEntity.child("ClearDecalOverlays").text().as_bool(true)) PED::CLEAR_PED_DECORATIONS(ep.Handle());
			...
			if (applyComps)
			{
				...
				for (auto& nodePedCompsObject = nodePedComps.first_child(); nodePedCompsObject; nodePedCompsObject = nodePedCompsObject.next_sibling())
				{
					...
					PED::SET_PED_COMPONENT_VARIATION(ep.Handle(), pedCompId, pedCompIdValueDrawable, pedCompIdValueTexture, 0);
				}
			}
			if (applyProps)
			{
				PED::CLEAR_ALL_PED_PROPS(ep.Handle());
			...
			//	std::vector<Spooner::SpoonerEntityWithInitHandle> vSpawnedAttachments;
			...
			//	{
			//		auto& e = Spooner::FileManagement::SpawnEntityFromXmlNode(nodeAttachment, vModelHashes);
			//		Spooner::EntityManagement::AttachEntity(e.e, ep, e.e.AttachmentArgs.boneIndex, e.e.AttachmentArgs.offset, e.e.AttachmentArgs.rotation);
			...
			//			}
			//			Spooner::Databases::EntityDb.push_back(e.e);
			//		}
			//		else
			//		{
