...
{
	if (!PED::CAN_PED_RAGDOLL(this->mHandle))
	{
		PED::SET_PED_CAN_RAGDOLL(this->mHandle, true);
	}
	PED::SET_PED_TO_RAGDOLL(this->mHandle, 5000, -1, 1, 1, 1, 0);
	PED::CREATE_NM_MESSAGE(false, (int)messageid);
	PED::GIVE_PED_NM_MESSAGE(this->mHandle);
}
//
...
PedHeadBlendData GTAped::HeadBlendData_get() const
{
	...
	PED::_GET_PED_HEAD_BLEND_DATA2(this->mHandle, &blendData);
	return blendData;
}
void GTAped::HeadBlendData_set(const PedHeadBlendData& blendData)
{
	PED::SET_PED_HEAD_BLEND_DATA(this->mHandle, blendData.shapeFirstID, blendData.shapeSecondID, blendData.shapeThirdID,
...
{
	Hash weap = 0;
	WEAPON::GET_CURRENT_PED_WEAPON(this->mHandle, &weap, true);
	return weap;
}
void GTAped::Weapon_set(Hash weaponHash)
{
	if (!WEAPON::HAS_PED_GOT_WEAPON(this->mHandle, weaponHash, false))
		WEAPON::GIVE_WEAPON_TO_PED(this->mHandle, weaponHash, 9999, true, false);
	WEAPON::SET_CURRENT_PED_WEAPON(this->mHandle, weaponHash, true);
}
...
{
	return PED::GET_PED_ARMOUR(this->mHandle);
}
void GTAped::Armour_set(int value)
{
	PED::SET_PED_ARMOUR(this->mHandle, value);
}

void GTAped::BlockPermanentEvents_set(bool value)
{
	PED::SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(this->mHandle, value);
}

bool GTAped::CanRagdoll_get() const
{
	return PED::CAN_PED_RAGDOLL(this->mHandle) != 0;
}
void GTAped::CanRagdoll_set(bool value)
{
	PED::SET_PED_CAN_RAGDOLL(this->mHandle, value);
}

void GTAped::CanSwitchWeapons_set(bool value)
{
	PED::SET_PED_CAN_SWITCH_WEAPON(this->mHandle, value);
}
...
{
	return PED::GET_PED_RELATIONSHIP_GROUP_HASH(this->mHandle);
}
void GTAped::RelationshipGroup_set(Hash grpHash)
{
	PED::SET_PED_RELATIONSHIP_GROUP_HASH(this->mHandle, grpHash);
}
void GTAped::RelationshipGroup_set(const std::string& grpName)
{
	PED::SET_PED_RELATIONSHIP_GROUP_HASH(this->mHandle, gethashkey::GET_HASH_KEY(grpName));
}
...
{
	PED::APPLY_PED_DAMAGE_PACK(this->mHandle, (PCHAR)damagePackName.c_str(), damageAmount, multiplier);
}
...
//		for (auto& c : *cc)
//		{
//			if (WEAPON::HAS_PED_GOT_WEAPON(this->mHandle, c.weaponHash, false))
//			{
//				wct.componentHashes.clear();
//				wct.weaponHash = c.weaponHash;
//				for (const auto& comp : c.components)
//				{
//					if (WEAPON::HAS_PED_GOT_WEAPON_COMPONENT(this->mHandle, c.weaponHash, comp.hash))
//					{
//						wct.componentHashes.push_back({ comp.hash, _0xF0A60040BE558F2D(this->mHandle, c.weaponHash, comp.hash) });
//					}
//				}
//				wct.tint = WEAPON::GET_PED_WEAPON_TINT_INDEX(this->mHandle, c.weaponHash);
//
//				result.push_back(wct);
//			}
//		}
//	}
//	if (WEAPON::HAS_PED_GOT_WEAPON(this->mHandle, GADGET_PARACHUTE, false))
//	{
//		wct.componentHashes.clear();
